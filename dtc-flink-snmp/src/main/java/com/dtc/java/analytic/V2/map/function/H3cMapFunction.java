    ...
    public DataStruct map(DataStruct event) throws Exception {
        ...
        if (LastCode.contains(".")) {
            ...
            if ("102_101_101_101_101".equals(lastCode) || "102_101_102_102_102".equals(lastCode) || "102_101_102_103_103".equals(lastCode)) {
                ...
                //板卡
                return new DataStruct(event.getSystem_name() + "|h3c_switch_0", event.getHost(), lastCode, nameCode, event.getNameCN(), event.getNameEN(), event.getTime(), event.getValue());
            ...
            if ("102_101_103_106_107".equals(lastCode) || "102_101_103_107_108".equals(lastCode) ||
                ...
                String nameCode = LastCode.split("\\.", 2)[1];
                //端口
                return new DataStruct(event.getSystem_name() + "|h3c_switch_1", event.getHost(), lastCode, nameCode, event.getNameCN(), event.getNameEN(), event.getTime(), event.getValue());
            }
        }
        return new DataStruct(event.getSystem_name() + "|h3c_switch_2", event.getHost(), event.getZbFourName() + "_" + event.getZbLastCode(), "", event.getNameCN(), event.getNameEN(), event.getTime(), event.getValue());
    }
}
